...
public class AcceptJoinWarHandler extends CommandBookHandler {
    ...
    public AcceptJoinWarHandler(NationPlugin plugin) {
        super(plugin);
    ...
    protected List<String> getAcceptedTitles() {
        return List.of("Accept Join War");
    }
    ...
    protected BookResult validate(Player player, Block lecternBlock, BookMeta meta) {
        ...
        if (!nationDAO.getLeaderUuid(nationUuid).equals(uuid)) {
            return sendErrorBook(lecternBlock, player, "Only the nation leader may accept join war requests.");
        }

        boolean inOwnTown = townDAO.getTownsByNationUuid(nationUuid)
                .stream()
                .anyMatch(town -> claimDAO.isLecternInTownChunk(town, lecternBlock));
        if (!inOwnTown) {
            return sendErrorBook(lecternBlock, player, "This lectern is not within your nation's territory.");
        }
    ...
    protected BookResult execute(Player player, Block lecternBlock, BookMeta meta) {
        ...
        if (targetNationName.isEmpty()) {
            return sendErrorBook(lecternBlock, player, "Target nation name is required on the first line.");
        }
        ...
        if (!requestsDAO.hasPendingRequestBetween(targetNationUuid, selfNationUuid, REQUEST_JOIN_WAR_DELIVERY_TYPE)) {
            return sendErrorBook(lecternBlock, player, "No pending join war request from " + targetNationName + ".");
        }

        int warId = warDAO.getOngoingWarIdByNation(selfNationUuid);
        if (warId == -1) {
            return sendErrorBook(lecternBlock, player, "Your nation is not currently in a war.");
        }

        warAlliesDAO.addNationToWar(warId, targetNationUuid, selfNationUuid.equals(warDAO.getWarById(warId).attackerNationUuid()) ? "attacker" : "defender");
        ...
        return sendSuccessBook(lecternBlock, player,
                ...
                "§lRequest Accepted§r\n" +
                        getDate() +
                        "\n\n" +
                        "§o" + targetNationName + "§r will now join your war effort.\n\nUnited, your forces grow stronger.",
                nationDAO.getNationName(selfNationUuid)
        );
    }
}
